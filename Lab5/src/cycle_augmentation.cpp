...
TSPSolution cycleAugmentation(const TSPData& data, const TSPSolution& solution){
  ...
  for(int i = 0; i < solution.pathA.size(); i++){
    visited[solution.pathA[i]] = true;
  }
  for(int i = 0; i < solution.pathB.size(); i++){
    visited[solution.pathB[i]] = true;
  }
  if(newSolution.pathA.size() < newSolution.pathB.size()){
    newSolution.pathA.swap(newSolution.pathB);
  }
  while(newSolution.pathA.size() > newSolution.pathB.size()){
    std::pair<int, int> partialSolutionB = findBestCycle(data, visited, newSolution.pathB);
    visited[partialSolutionB.second] = true;
    newSolution.pathB.insert(newSolution.pathB.begin() + partialSolutionB.first, partialSolutionB.second); 
  }
  
  while(visited != std::vector<bool>(data.coordinates.size(), true)) {
    std::pair<int, int> partialSolutionA = findBestCycle(data, visited, newSolution.pathA);
    visited[partialSolutionA.second] = true;
    newSolution.pathA.insert(newSolution.pathA.begin() + partialSolutionA.first, partialSolutionA.second);
  ...
  newSolution.distance = calculateDistanceCycle(data, newSolution.pathA) + calculateDistanceCycle(data, newSolution.pathB);
  return newSolution;
